<template>
  <div>
    ...
    <n-modal v-model:show="showModal" :title="isEdit ? '编辑平台' : '新增平台'" preset="card" style="width: 500px" :mask-closable="false">
      <n-form ref="formRef" :model="formData" :rules="rules">
        <n-form-item path="name" label="平台名称">
          <n-input v-model:value="formData.name" placeholder="请输入平台名称" />
        </n-form-item>
        <n-form-item path="mark" label="平台标识">
          <n-input v-model:value="formData.mark" placeholder="请输入平台标识" :disabled="isEdit" />
        </n-form-item>
...
import { getGamePlatformList, createGamePlatform, updateGamePlatform, deleteGamePlatform } from '../../api/game'
import { formatTime } from '../../utils/format'

const { loading, tableData, pagination, search, resetSearch, handlePageChange, handlePageSizeChange } = useTable(getGamePlatformList)
...
async function handleSubmit() { if (await submit(formData, createGamePlatform, updateGamePlatform)) search({ keyword: searchKeyword.value, status: searchStatus.value ?? -1 }) }
async function onDelete(id) { if (await doDelete(id, deleteGamePlatform)) search({ keyword: searchKeyword.value, status: searchStatus.value ?? -1 }) }

async function handleStatusChange(row, val) {
  try {
    await updateGamePlatform(row.id, { ...row, status: val ? 1 : 0 })
    row.status = val ? 1 : 0
