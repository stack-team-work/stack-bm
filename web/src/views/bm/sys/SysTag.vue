    ...
    <n-card :bordered="false">
      <div class="search-bar">
        <n-space :size="12" align="center" wrap>
          ...
          <n-select v-model:value="searchStatus" :options="statusOptions" placeholder="状态" clearable style="width: 110px" @update:value="doSearch" />
    ...
    <n-modal v-model:show="showModal" :title="isEdit ? '编辑标签' : '新增标签'" preset="card" style="width: 560px" :mask-closable="false">
      <n-form ref="formRef" :model="formData" :rules="rules" label-placement="left" label-width="100">
        <n-grid :cols="2" :x-gap="16">
          ...
          <n-grid-item :span="2">
            <n-form-item path="remark" label="备注" label-placement="left">
...
import { getSysTagList, createSysTag, updateSysTag } from '../../../api/bm/sys'
import { formatTime } from '../../../utils/format'
import TableActions from '../../../components/TableActions.vue'

const { loading, tableData, pagination, search, handlePageChange, handlePageSizeChange } = useTable(getSysTagList)
...
const typeOptions = computed(() => options('sys_tag_type'))
const statusOptions = computed(() => options('status'))
const typeLabel = { 1: '素材类型', 2: '需求套路', 3: '其他' }

const formData = reactive({ type: 1, name: '', remark: '', status: 1 })
function resetForm() { Object.assign(formData, { type: 1, name: '', remark: '', status: 1 }) }
const rules = { name: [{ required: true, message: '请输入标签名称', trigger: 'blur' }] }

const columns = [
  { title: 'ID', key: 'id', width: 60 },
  { title: '名称', key: 'name' },
  { title: '类型', key: 'type', width: 110, render: (row) => typeLabel[row.type] || row.type },
  { title: '备注', key: 'remark' },
  ...
  { title: '操作', key: 'actions', width: 100, render: (row) => h(TableActions, { row, edit: () => handleEdit(row) }) },
]
...
function handleEdit(row) { resetForm(); formData.type = row.type; formData.name = row.name; formData.remark = row.remark; formData.status = row.status; openEdit(row) }
async function handleSubmit() { if (await submit(formData, createSysTag, updateSysTag)) doSearch() }

async function handleStatusChange(row, val) {
  try {
    await updateSysTag(row.id, { ...row, status: val ? 1 : 0 })
    row.status = val ? 1 : 0
