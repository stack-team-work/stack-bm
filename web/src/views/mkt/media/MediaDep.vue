<template>
  <div>
    ...
    <n-modal v-model:show="showModal" :title="isEdit ? '编辑部门' : '新增部门'" preset="card" style="width: 560px" :mask-closable="false">
      <n-form ref="formRef" :model="formData" :rules="rules" label-placement="left" label-width="100">
        <n-form-item path="name" label="部门名称">
          <n-input v-model:value="formData.name" placeholder="请输入部门名称" />
        </n-form-item>
        <n-form-item path="mark" label="部门标识">
          <n-input v-model:value="formData.mark" placeholder="请输入部门标识（留空自动生成）" :disabled="isEdit" />
        </n-form-item>
      </n-form>
      <template #footer>
        <n-space justify="end">
...
import { getMediaDepList, createMediaDep, updateMediaDep, deleteMediaDep } from '../../../api/mkt/media'
import { formatTime } from '../../../utils/format'
import { useDict } from '../../../composables/useDict'
import TableActions from '../../../components/TableActions.vue'

const { loading, tableData, pagination, search, handlePageChange, handlePageSizeChange } = useTable(getMediaDepList)
...
async function handleSubmit() { if (await submit(formData, createMediaDep, updateMediaDep)) doSearch() }
async function onDelete(id) { if (await doDelete(id, deleteMediaDep)) doSearch() }
async function handleStatusChange(row, val) {
  try { await updateMediaDep(row.id, { ...row, status: val ? 1 : 0 }); row.status = val ? 1 : 0; message.success('状态已更新') } catch { message.error('更新失败') }
}
