<template>
  <div>
    ...
    <n-modal v-model:show="showModal" :title="isEdit ? '编辑代理' : '新增代理'" preset="card" style="width: 560px" :mask-closable="false">
      <n-form ref="formRef" :model="formData" :rules="rules" label-placement="left" label-width="100">
        <n-grid :cols="2" :x-gap="16">
          <n-form-item-gi path="subject_id" label="所属主体">
            <n-select v-model:value="formData.subject_id" :options="subjectOptions" placeholder="请选择主体" clearable />
          </n-form-item-gi>
          <n-form-item-gi path="name" label="代理名称">
            <n-input v-model:value="formData.name" placeholder="请输入代理名称" />
          </n-form-item-gi>
          <n-form-item-gi path="mark" label="代理标识">
            <n-input v-model:value="formData.mark" placeholder="请输入代理标识（留空自动生成）" :disabled="isEdit" />
          </n-form-item-gi>
          <n-grid-item>
...
import { getMediaAgentList, createMediaAgent, updateMediaAgent, deleteMediaAgent } from '../../../api/mkt/media'
...
const rules = {
  ...
  subject_id: [{ required: true, type: 'number', message: '请选择主体', trigger: 'change' }],
}
const columns = [
  { title: 'ID', key: 'id', width: 60 },
  { title: '所属主体', key: 'subject_id', width: 130, render: (row) => { const s = subjectOptions.value.find(o => o.value === row.subject_id); return s ? s.label : '' } },
  { title: '代理名称', key: 'name' },
  { title: '代理标识', key: 'mark' },
...
function doSearch() { search({ keyword: searchKeyword.value, subject_id: searchSubjectId.value ?? 0, status: searchStatus.value ?? -1 }) }
function handleAdd() { resetForm(); open() }
function handleEdit(row) { resetForm(); formData.subject_id = row.subject_id; formData.name = row.name; formData.mark = row.mark; formData.status = row.status; openEdit(row) }
async function handleSubmit() { if (await submit(formData, createMediaAgent, updateMediaAgent)) search({ keyword: searchKeyword.value, subject_id: searchSubjectId.value ?? 0, status: searchStatus.value ?? -1 }) }
async function onDelete(id) { if (await doDelete(id, deleteMediaAgent)) search({ keyword: searchKeyword.value, subject_id: searchSubjectId.value ?? 0, status: searchStatus.value ?? -1 }) }
...
onMounted(async () => { await loadDict(); subjectOptions.value = await loadOptions('media_subject'); search({}) })
</script>
