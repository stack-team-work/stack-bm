...
async function handleCopy(row) {
  ...
  if (!name) return
  try { await copyKsAudienceTemplate({ id: row.id, template_name: name }); message.success('复制成功'); search({ keyword: searchKeyword.value }) }
  catch (err) { message.error(err.message || '复制失败') }
}
async function onDelete(id) { if (await doDelete(id, deleteKsAudienceTemplate)) search({ keyword: searchKeyword.value }) }
