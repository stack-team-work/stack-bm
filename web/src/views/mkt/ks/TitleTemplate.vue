...
const columns = [
  ...
  { title: '操作', key: 'actions', width: 180, render: (row) => h(NSpace, null, { default: () => [
    h(NButton, { size: 'tiny', onClick: () => router.push(`/ks-ads/title-template/edit/${row.id}`) }, { default: () => '编辑' }),
...
async function handleCopy(row) {
  ...
  if (!name) return
  try { await copyKsTitleTemplate({ id: row.id, template_name: name }); message.success('复制成功'); search({ keyword: searchKeyword.value }) }
  catch (err) { message.error(err.message || '复制失败') }
}
async function onDelete(id) { if (await doDelete(id, deleteKsTitleTemplate)) search({ keyword: searchKeyword.value }) }
