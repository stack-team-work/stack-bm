    ...
    <n-card style="margin-top: 16px">
      <n-form ref="formRef" :model="formData" :rules="rules" label-placement="left" label-width="100">
        ...
        <n-space v-for="(item, i) in formData.description" :key="i" align="center" style="margin-bottom: 8px">
          <n-input v-model:value="formData.description[i]" placeholder="请输入描述" style="width: 420px" />
        ...
        <n-card v-for="(m, i) in formData.title_materials" :key="i" size="small" style="margin-bottom: 12px">
          ...
          <n-space v-for="(s, j) in m.smart_title_list" :key="j" align="center" style="margin-top: 8px">
            <n-select v-model:value="s.type" :options="dict('bili_title_word_type')" placeholder="词包类型" style="width: 180px" />
            <n-input v-model:value="s.default_value" placeholder="默认值" style="width: 280px" />
...
import { getBiliTitleTemplateDetail, createBiliTitleTemplate, updateBiliTitleTemplate } from '../../../api/mkt/bili'
...
function goBack() { router.push({ path: '/bili-ads', query: { tab: 'title' } }) }
...
function fill(data) {
  ...
  formData.description = (data.description || []).filter(Boolean)
...
async function handleSubmit() {
  ...
  try {
    ...
    if (isEdit.value) { await updateBiliTitleTemplate(editId.value, data); message.success('更新成功') }
    else { await createBiliTitleTemplate(data); message.success('创建成功') }
    goBack()
...
onMounted(async () => {
  ...
  if (id) {
    ...
    editId.value = id
    try {
      const res = await getBiliTitleTemplateDetail(id)
