...
import (
	...
	tcTool "stack-bm/internal/service/mkt/tc/tool"
...
type ToolHandler struct {
	campaign *tcTool.CampaignTool
	unit     *tcTool.UnitTool
}

func NewToolHandler() *ToolHandler {
	auth := oauth.NewManagerAuth()
	return &ToolHandler{
		campaign: tcTool.NewCampaignTool(auth),
		unit:     tcTool.NewUnitTool(auth),
	}
}
...
func (h *ToolHandler) dispatchCampaign(action string, req *tcTool.ToolRequest) (interface{}, error) {
	switch action {
	case "open":
...
func (h *ToolHandler) dispatchUnit(action string, req *tcTool.ToolRequest) (interface{}, error) {
	switch action {
	case "open":
