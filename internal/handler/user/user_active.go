...
type UserActiveHandler struct {
	service *userSvc.UserActiveService
}

func NewUserActiveHandler() *UserActiveHandler {
	return &UserActiveHandler{service: userSvc.NewUserActiveService()}
}

func (h *UserActiveHandler) GetList(c *gin.Context) {
	var req struct {
		Page    int    `json:"page"`
		Size    int    `json:"size"`
