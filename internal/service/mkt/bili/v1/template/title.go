...
func NewTitleService() *TitleService {
	return &TitleService{repo: biliRepo.NewTitleTemplateRepository()}
}

func (s *TitleService) Create(doc *biliModel.TitleTemplate, adminID int) error {
	if doc.TemplateName == "" {
...
func (s *TitleService) FindPage(page, size int, keyword string) ([]biliModel.TitleTemplate, int64, error) {
	if page < 1 {
		page = 1
	}
	if size < 1 {
...
func (s *TitleService) FindByID(id string) (*biliModel.TitleTemplate, error) {
	return s.repo.FindByID(id)
}

func (s *TitleService) Update(id string, doc *biliModel.TitleTemplate) error {
