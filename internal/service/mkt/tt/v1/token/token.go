...
// TokenService 头条渠道 token 服务：解析本渠道 access_token，
...
// AccessToken 渠道账户记录ID(media_accounts.id) -> access_token（层级同步用；
// 头条V3暂无钱包接口，账户余额同步为占位）
...
// AccessTokenByUID 平台广告主UID -> 定位渠道账户行 -> access_token
