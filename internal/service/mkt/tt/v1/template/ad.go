...
func NewAdService() *AdService {
	return &AdService{repo: ttRepo.NewAdTemplateRepository()}
}

func now() string { return time.Now().Format("2006-01-02 15:04:05") }

func (s *AdService) Create(doc *ttModel.AdTemplate, adminID int) error {
	if doc.TemplateName == "" {
...
func (s *AdService) FindPage(page, size int, keyword string) ([]ttModel.AdTemplate, int64, error) {
	if page < 1 {
		page = 1
	}
	if size < 1 {
...
func (s *AdService) FindByID(id string) (*ttModel.AdTemplate, error) {
	return s.repo.FindByID(id)
}

func (s *AdService) Update(id string, doc *ttModel.AdTemplate) error {
