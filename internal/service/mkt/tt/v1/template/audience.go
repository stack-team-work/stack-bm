...
func NewAudienceService() *AudienceService {
	return &AudienceService{repo: ttRepo.NewAudienceTemplateRepository()}
}

func (s *AudienceService) Create(doc *ttModel.AudienceTemplate, adminID int) error {
	if doc.TemplateName == "" {
...
func (s *AudienceService) FindPage(page, size int, keyword string) ([]ttModel.AudienceTemplate, int64, error) {
	if page < 1 {
		page = 1
	}
	if size < 1 {
...
func (s *AudienceService) FindByID(id string) (*ttModel.AudienceTemplate, error) {
	return s.repo.FindByID(id)
}

func (s *AudienceService) Update(id string, doc *ttModel.AudienceTemplate) error {
