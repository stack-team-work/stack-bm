...
func NewTitleService() *TitleService {
	return &TitleService{repo: ttRepo.NewTitleTemplateRepository(), wordRepo: ttRepo.NewWordListRepository()}
}

var wordPlaceholderRe = regexp.MustCompile(`\{([^\{\}]+)\}`)

func (s *TitleService) validateMaterials(materials []ttModel.TtTitleMaterial) error {
...
func (s *TitleService) Create(doc *ttModel.TitleTemplate, adminID int) error {
	if doc.TemplateName == "" {
...
func (s *TitleService) FindPage(page, size int, keyword string) ([]ttModel.TitleTemplate, int64, error) {
	if page < 1 {
		page = 1
	}
	if size < 1 {
...
func (s *TitleService) FindByID(id string) (*ttModel.TitleTemplate, error) {
	return s.repo.FindByID(id)
}

func (s *TitleService) Update(id string, doc *ttModel.TitleTemplate) error {
