...
type AdTemplateService struct {
	repo *ttRepo.AdTemplateRepository
}

func NewAdTemplateService() *AdTemplateService {
	return &AdTemplateService{repo: ttRepo.NewAdTemplateRepository()}
}

func now() string { return time.Now().Format("2006-01-02 15:04:05") }

func (s *AdTemplateService) Create(doc *ttModel.AdTemplate, adminID int) error {
	if doc.TemplateName == "" {
...
func (s *AdTemplateService) FindPage(page, size int, keyword string) ([]ttModel.AdTemplate, int64, error) {
	if page < 1 {
		page = 1
	}
	if size < 1 {
...
func (s *AdTemplateService) FindByID(id string) (*ttModel.AdTemplate, error) {
	return s.repo.FindByID(id)
}

func (s *AdTemplateService) Update(id string, doc *ttModel.AdTemplate) error {
